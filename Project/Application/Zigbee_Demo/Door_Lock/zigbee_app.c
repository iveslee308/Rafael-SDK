    ...
    for (;;)
    {
        ...
        if (sys_queue_recv(&app_msg_q, &t_app_q, 0) != SYS_ARCH_TIMEOUT)
        {
            if (t_app_q.event == APP_QUEUE_ISR_BUTTON_EVT)
            {
                ...
                {
                    zigbee_zcl_attr_set_t *pt_attr_set;
                    ZIGBEE_ZCL_ATTR_SET(pt_attr_set, 2, ZB_ZCL_CLUSTER_ID_DOOR_LOCK, ZB_ZCL_CLUSTER_SERVER_ROLE, ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_ID, 0, 1);
                    if (pt_attr_set)
                    {
                        pt_attr_set->value[0] = ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_LOCKED;
                        zigbee_zcl_attr_set(pt_attr_set, 1);
                        sys_free(pt_attr_set);
                    }
                }
                break;
                case BSP_EVENT_BUTTONS_1:
                {
                    ...
                    if (pt_attr_set)
                    {
                        pt_attr_set->value[0] = ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_UNLOCKED;
                        zigbee_zcl_attr_set(pt_attr_set, 1);
                        sys_free(pt_attr_set);
                    }
                }
